...
pacman::p_load(
  ...
  officer,        # 辅助函数helper functions for tables
  tidyverse)      # 数据管理、摘要和可视化
...
本节将介绍如何使用**flextable**包制作用于演示的统计报表。这些表格可以插入到幻灯片、网页、PDF或Word文档中。
...
在使用**flextable**之前，您必须将汇总表创建为数据框。可以用[Descriptive tables]和[Pivoting data]章节中所提到的方法，如制表、交叉制表、长宽表转换和描述性统计来得到描述性统计表。得到的数据框将被传递给**flextable**，并可对其进行不同显示格式的设置。
...
还有许多其他的R包可以用来制作用于展示的表-在本章节我们主要介绍**flextable**包。除此，使用**knitr**包及其' kable() '函数的示例可以在[Contact Tracing]页面中找到。**DT**包在页面[dashboard with Shiny]中有所提及。其他的R包例如**GT**和**huxtable**在[Suggested packages]页面中提到。
...
安装和加载**flextable**。在本书中，我们推荐使用` p_load()`  from **pacman**，它会在必要时安装包并加载它以供使用。你也可以用` library() `从**base** R中加载包。

```{r}
pacman::p_load(
  rio,            # 导入/导出
  here,           # 文件路径
  flextable,      # 制作HTML格式的表格 
  officer,        # 制表辅助R包
  tidyverse)      # 数据管理、摘要可视化
...
首先，我们从模拟埃博拉疫情例子中导入已处理的病例列表。<a href='https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'>点击下载linelist</a> (.rds文件)
使用' Import() '函数从**rio**包中导入数据(它处理许多文件类型，如.xlsx， .csv， .rds -详情请参阅[Import and export]页面)。
...
在开始使用**flextable**之*前*，您需要将表创建为数据框。参见[Descriptive tables]和[Pivoting data]的页面，学习如何使用诸如**janitor**和**dplyr**等包创建数据框。您必须按照您所希望显示的行和列来排列内容。然后，数据框将被传递给**flextable**，（通过设置不同的参数）来设置数据框的颜色、标题、字体等。
...
下面是来自[Descriptive tables]页面的一个示例，该示例将`linelist`转换为按医院汇总患者结果和CT值的数据框，最后一行为总计。汇总结果被输出保存为 `table`。
...
table <- linelist %>% 
  ...
  # Get summary values per hospital-outcome group
  ###############################################
  group_by(hospital, outcome) %>%                      # 数据分类
  summarise(                                           # Create new summary columns of indicators of interest
    N = n(),                                            # Number of rows per hospital-outcome group     
    ct_value = median(ct_blood, na.rm=T)) %>%           # median CT value per group
...
为了创建和管理**flextable**对象，我们首先通过`flextable()`函数将数据`table`传递给`my_table`。
...
这样做之后，我们可以通过更多的**flextable**函数逐步管道' my_table '对象。

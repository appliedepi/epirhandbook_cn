...
转换为Dat类时必须要小心，[处理日期]章节已经介绍了几种方法。通常，原始日期值必须采用相同的格式才能正确转换（例如“MM/DD/YYYY”或“DD MM YYYY”）。转换为Date类后，要检查数据以确认每个值都已正确转换。
...
指定拟操作的列到`.cols =`参数和拟应用的函数到`.fns =`参数。提供给`.fns`函数的任何其他参数都可以放进`across()`函数，并以英文逗号与`.fns`分隔，置于其后。

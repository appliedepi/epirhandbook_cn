...
<span style="color: red;">**_危险：_** 如果用`readWorkbook()`导入合并单元格数据，您最终会得到重复的列名，您需要手动修复 - R 不能很好地处理重复的列名！您可以通过引用其位置（例如第5列）来重命名列，如手动列名清洗部分所述。</span>

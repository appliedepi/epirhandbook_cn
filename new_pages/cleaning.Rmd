...
为阐述数据清洗，本节首先导入原始病例一览表数据集，然后逐步演示清洗过程。在 R 代码中，这个过程可以用一个“管道”链完成：应用管道操作符“%>%”将数据从一个操作传递到下一个操作。
...
本手册推荐使用[**tidyverse**](https://www.tidyverse.org/)系列 R 包中的函数。下面列出了本节中演示的基本 R 函数。
...
还可以引用其他列中的值来换算。下面，创建了一个新列`bmi`来保存每个病例的体重指数 (BMI) - 使用`ht_cm`和`wt_kg`两列数据和公式BMI = kg/m^2换算。
...
如果创建多个新列，请用逗号和新行分隔每一列。下面是换算新列的示例，包括那些用**stringr**包的 `str_glue()` 函数由其他列的值组合而成的新列（请参阅[字符和字符串]章节内容）。
...
new_col_demo <- linelist %>%                       
  mutate(
    new_var_dup    = case_id,             # 新列 = 重复/复制其他已有列
    new_var_static = 7,                   # 新列 = 所有值都相同
    new_var_static = new_var_static + 5,  # 可以直接改写一列，且可以用其他变量换算
    new_var_paste  = stringr::str_glue("{hospital} on ({date_hospitalisation})") # 新列 = 与其他列的值粘贴在一起
    ) %>% 
  select(case_id, hospital, date_hospitalisation, contains("new"))        # 为演示方便，仅显示新列
```


Review the new columns. For demonstration purposes, only the new columns and the columns used to create them are shown:  

查看新列，为方便演示，这里仅显示新列和用于创建新列所使用的列。
...
<span style="color: darkgreen;">**_提示：_** `mutate()` 函数 有一个变体`transmute()`。此函数可像`mutate()` 一样添加一个新列，但也会丢弃/删除括号中未提及的所有其他列。</span>
...
包含日期、数字或逻辑值 (TRUE/FALSE) 值的列只有在正确分类时才会按预期运行。字符类的“2”和数值类的“2”是有区别的！

有一些方法可以在导入命令期间设置列类，但这通常很麻烦。请参阅[R语言基础]章节有关对象类部分以了解有关转换对象和列类的更多信息。

首先，让我们对重要的列进行一些检查，看看它们是否是正确的类。我们在开始运行`skim()`时也可以查看到这些信息。

目前，`age`列的类是字符。为了进行定量分析，我们需要将这些数字识别为数值！
...
The class of the `date_onset` column is also character! To perform analyses, these dates must be recognized as dates! 
 ...
 `date_onset`列的类也是字符！如果开展分析，这些日期要被识别为日期类。
...
要解决此问题，请使用`mutate()`来转换、重新定义列。可以引用拟转换的列本身，此列原位转换为不同的类。这里有一个简单示例，转换并使得`age`列为Numeric类：
...
以类似的方式，您可以使用`as.character()`和`as.logical()`转换相应的类。要转换为 Factor 类，您可以使用**base R**里的`factor()`函数或**forcats**包里的`as_factor()`函数。请参阅[因子]章节里有关此内容的更多信息。

转换为Date类时必须要小心，[处理日期]章节已经介绍了几种方法。通常，原始日期值必须采用相同的格式才能正确转换（例如“MM/DD/YYYY”或“DD MM YYYY”）。转换为Date类后，要检查数据以确认每个值都已正确转换。
...
如果数据框已经*分组*（参见[数据分组]章节），则使用`mutate()`时可能与在未分组数据框中运行方式不同。任何汇总函数，如`mean()`、`median()`、`max()`等将按组计算，而不是按所有行计算。
...
关于`mutate ()`在分组数据框的更多应用信息可参阅[tidyverse mutate 文档](https://dplyr.tidyverse.org/reference/mutate.html)。 



### Transform multiple columns {#clean_across .unnumbered}
### 转换多列 {#transform_multiple}
...
经常要编写一些简洁的代码，以便一次将相同的转换应用于多个列。应用**dplyr**包的`across()`函数（同时也封装 在**tidyverse**包里）可以将一种转换操作一次性应用到多列。`across()`可以用在**dplyr**包的任何函数里，但比较常用的是`select()`，`mutate()`，`filter()`或`summarise()`。`summarise()`使用说明可参阅[描述性表格]章节。

指定拟操作的列到`.cols =`参数和拟应用的函数到`.fns =`参数。提供给`.fns`函数的任何其他参数都可以放进`across()`函数，并以英文逗号与`.fns`项分隔，置于其后。
...
将拟操作的列指定给`.cols =`参数，可以单独指定，也可以使"tidyselect"辅助函数批量指定。将拟应用的函数指定给`.fns =`参数。请注意使用下面演示的函数模式，函数名后*没有*括号"( )"。

此处`as.character()`转换操作全部应用于`across()`指定的特定列。
...
"tidyselect"辅助函数可用来帮助指定列，细节已经在列选择和重排序部分介绍。这些函数包括：`everything()`, `last_col()`, `where()`, `starts_with()`, `ends_with()`, `contains()`, `matches()`, `num_range()` 和 `any_of()`。

这里是一个如何一次把**所有列**更改成字符类的操作示例。
...
将名称包含字符串“date”的所有列转换为字符（注意逗号和括号的位置）：
...
下面是对当前属于 POSIXct 类（显示时间戳的原始日期时间类）的列（也就是说，所有被`is.POSIXct()`函数计算为TRUE的列）进行换算的示例。用`as.Date()`函数将这些列转换为普通的Date类。
...
* 请注意在`across()`中也可以应用`where()`函数，此时`is.POSIXct`计算结果或为TRUE或为FALSE。  
* 请注意`is.POSIXct()`来自**lubridate**包，而其他类似的"is"函数像 `is.character()`, `is.numeric()`和`is.logical()`均来自**base R**。  
...
可以用`?across`阅读文档以了解有关如何向`across()`提供函数。一些总结点：可在某一列上运行的函数适用的多种指定方法均适用于`across()`，甚至可以自定义函数：
* 可仅提供函数名称（例如`mean`或`as.character`）
* 可提供`purrr`风格函数（例如`~ mean(.x, na.rm = TRUE)`）（请参阅[此章节](https://epirhandbook.com/iteration-loops-and-lists.html#iteration-loops-and-lists)）
* 可以列表的形式提供多个函数（例如`list(mean = mean, n_miss = ~ sum(is.na(.x))`）。
* 如果提供多个函数，则每个输入列将返回多个转换后的列，这些列具有独特名称，格式为`col_fn`。可调整`.names =`参数用**glue**语法（其中 `{.col}` 和 `{.fn}` 代表指定的列和函数）改变新列的命名方式（请参阅[字符和字符串]章节）。

以下是一些有关`across()`使用的在线资源：[创作者Hadley Wickham 的想法/基本原理](https://www.tidyverse.org/blog/2020/04/dplyr-1-0-0-colwise/)
...
此**dplyr**包中的函数查找每个位置的第一个非缺省值，然后按照指定的顺序使用第一个可用值“填充”缺省值。

这是一个*数据框范围之外*的示例：假设有两个向量，一个包含患者的检测发现时所在的村庄，另一个包含患者居住的村庄。您可以使用coalesce为每个索引选取第一个非缺失值：
```{r}
...
这对于数据框列同样有效：该函数将指定（或按顺序）列中的第一个非缺省值分配给同一行存在缺省值的其他列作为其新值。
...
这是一个“逐行”操作示例。更复杂的逐行计算，请参阅逐行计算章节。



### Cumulative math {.unnumbered}
### 累计数学 {#cumulative_math}
...
当核算数据框各列目前的累积总和/累积均数/累积最小值/累积最大值，并产生一个新列，请使用以下函数：
...
```
```{r}
sum(c(2,4,15,10))     # 仅返回一个数
cumsum(c(2,4,15,10))  # 每一步返回一个累积总和
```
...
这可以为数据框创建新列。例如，要计算一起暴发疫情中每天累计病例数，可考虑如下代码：
...
```
```{r, warning=F, message=F}
cumulative_case_counts <- linelist %>%  # 以病例一览表开始
  count(date_onset) %>%                 # 计算每天的行数，存放在列名为"n"的新列中   
  mutate(cumulative_cases = cumsum(n))  # 创建新列，为每行计算累积总和
```
...
See the page on [Epidemic curves] for how to plot cumulative incidence with the epicurve.  
用累积发病数画流行曲线的方法请参阅[流行病学曲线]
还可参阅: `cumsum()`, `cummean()`, `cummin()`, `cummax()`, `cumany()`, `cumall()`等函数。 
...
要使用**base** R来定义新列（或重新定义列），一般是写出数据框名，然后用`$`将*新*列（或拟修改的列）名连接起来。建议使用赋值运算符`<-`来定义新值。请记住，在使用`base` R时，必须每次在列名之前明确数据框名称（例如`dataframe$column`）。以下是使用`base` R创建`bmi`列的示例：
...
手动更改值，您可以在mutate()函数内使用recode()函数。

假设数据中有一个无意义的日期（例如“2014-14-15”）：您可以在原始源数据中手动修复日期，或者，您可以通过`mutate()`和`recode()`的数据清洗流水线更改。后者对于想理解或重复您的分析的人来说更加透明和可重复。
...
`mutate()`上面的行可以理解为：“将`date_onset`原位转换成`date_onset`，并把原值更改为新”。请注意，此recode()中的函数模式（旧 = 新）与大多数R模式（新 = 旧）相反。R 开发社区正在努力修正这一点。

**这是重新编码一列中的多个值的另一个示例。**

在`linelist`的"hostpital"列也必须清洗：其中有几种不同的拼写和许多缺省值。
...
```{r}
table(linelist$hospital, useNA = "always")  # 打印所有独特值的表格，包括缺省值
```
...
`recode()`将“hospital”列原位重定义成新的“hospital”列，并仅修改指定的值。每行后不要忘记逗号。
...
<span style="color: darkgreen;">**_提示：_** 等号前后的空格数无关紧要。通过对齐所有或大多数行的 = 使您的代码更易于阅读。另外，可以考虑添加一行对应位置的注释，注明哪一边是原值，哪一边是新值。</span>  

<span style="color: darkgreen;">**_提示：_** 有时数据集中存在*空白*字符值（不被识别为R的缺省值 -`NA`）。可以使用中间没有空格的双引号 （""） 引用此值。</span>  
...
下面演示如何使用逻辑和条件重新编码列中的值：

* 简单逻辑：使用`replace()`, `ifelse()`和`if_else()`
* 复杂逻辑：使用`case_when()`
...
要使用简单逻辑条件重新编码，可以在`mutate()`内使用`replace()`。 `replace()`是**base** R的函数。使用逻辑条件指定要更改的行的一般语法是：

`mutate(拟处理行 = replace(拟处理行, 行选取条件, 新值))`

一种常见的使用`replace()`的情况是`使用唯一的行标识符仅更改一行中的一个值`。下面，将`case_id`列值为“2195”所在行的性别值更改为“女性”。
...
使用**base** R语法和索引括号`[ ]`的等效命令如下。它读作“将`linelist`数据框`case_id`值为'2195'所有行的`gender`列值更改为'女性'”。
...
另一个用于简单逻辑的工具是`ifelse()`及类似函数`if_else()`。但是，在大多数情况下，对于重新编码，`case_when()`使用起来更清晰（详情如下）。这些“if else”命令是`if`和`else`编程语句的简化版本。一般语法是：
`ifelse(逻辑条件, 条件为真时应返回的值, 条件为假时应返回的值)`
...
下面，`source_known`被的换算出来，如果`source`列*不是*缺省值，则其值是“known”；否则为“unknown”。
...
`if_else()`则是`dplyr`包中用于处理日期的一个特殊版本。请注意，如果条件'TRUE' 时返回值是日期，则 'FALSE'返回值也必须限定为日期，如果要返回缺省值则应使用特殊值`NA_real_`而不是`NA`。
...
**避免将很多ifelse命令行串在一起使用……请改用`case_when()`！** `case_when()` 更易阅读且少出错误。
...
在数据框范围外，如果要让代码中使用的对象切换取值，请考虑使用**base** R的`switch()`。


### Complex logic {#clean_case_when .unnumbered}  
### 复杂逻辑 {#clean_case_when .unnumbered}  
...
如果要重新编码成很多新组，或者要使用复杂的逻辑语句重码值，请使用**dplyr**包的`case_when()`。此函数评估数据框中的每一行是否满足指定条件，并分配正确的新值。

case_when()命令由以“波浪号”`~`分隔开来的右侧 (RHS) 和左侧 (LHS) 的语句组成，产生的新值在每个语句的右侧。语句用逗号分隔。

例如，下面我们用列`age`和`age_unit`创建一个新列`age_years`：
...
linelist <- linelist %>% 
  mutate(age_years = case_when(
            age_unit == "years"  ~ age,       # 如果年龄单位是年
            age_unit == "months" ~ age/12,    # 如果年龄单位是月
            is.na(age_unit)      ~ age,       # 如果年龄单位缺省，则默认为年
            TRUE                 ~ NA_real_)) # 其他情况赋值为缺省值
```
...
`case_when()`语句将按照逻辑条件编写顺序自上而下对数据框逐行应用/评估。如果某行用上位逻辑条件评估为`TRUE`的话，则会分配右侧值；其他余下的逻辑条件不会再去评估该行。因此，最好先编写最具体的条件，最后编写最通用的条件。
...
除了那些逻辑条件行之外，最后一行建议在左侧设置`TRUE`以框定那些未符合之前所有逻辑条件的行。那么这句的右侧应该赋值为“待核查”或缺省值。
...
<span style="color: red;">**_危险：_** **右侧的值必须是同一类型** - 数值、字符、日期、逻辑等任意一种。 要赋缺省值（`NA`），需用`NA`的变体，如`NA_character_`, `NA_real_` （用于数值或日期时间型）和 `as.Date(NA)`。更多信息参阅[处理日期]章节。</span>  
...
有关识别和处理缺失值的更多技巧，请参阅[缺省数据]章节。例如，用于逻辑判断缺省值 的函数`is.na()`。
...
可以在`mutate()`函数里使用**dplyr**包的`replace_na()`函数，将诸如“Missing”一类的特定缺省值替换成缺省值（`NA`）。请注意，它的使用方式与上述的`recode`相同- 必须在`replace_na()`内重复提供拟处理的变量名称。
...
这是专门处理因子的**forcats**包中的一个函数。因子是 R 处理有序值，如`c("First", "Second", "Third")`或设置值（例如医院）在表格和图中的出现顺序的方式。请参阅[因子]章节。
...
如果用`replace_na()`把数据中的因子类的`NA`值更改为“Missing”，会出现以下错误提示：`非法的因子水平，将产生缺省值（invalid factor level, NA generated）`。要为因子添加“Missing”值，但未将其定义成其因子水平的话，该操作将被拒绝。

解决此问题的最简单方法是使用`forcats`包里的`fct_explicit_na()`函数。该函数可将列转换为因子，并将`NA`值转换为字符“(Missing)”。
...
较慢的替代方法是使用`fct_expand()`添加因子级别，并转换为缺省值。
...
要将*特定值*转换为`NA`，请使用**dplyr**包里的`na_if()`。下面的命令执行与`replace_na()` 的相反操作。在下面的示例中，`hospital`列中“缺省”的任何值都将转换为`NA`。
...
注意：`na_if()` **不能作为逻辑条件**（如“所有值>99”），可用`replace()` 或 `case_when()` 。
...
应用**linelist**包的`clean_variable_spelling()`函数，导入*清洗字典*来清洗数据框数据。该包是由[流行病联盟RECON](https://github.com/reconhub/linelist)研发制作的。
...
1）创建一个3行的清洗字典：
    * 一个“from”列（不正确值）
    * 一个“to”gqj （正确值）
    * 一个拟处理的指定列（或者用“.global”应用于全部列）
    
注意： .global 字典条目将被列特定的字典条目覆盖。
...
2）向R导入字典文件。这个示例可以在[手册和数据下载]章节下载。
...
3）将原始linelist传递给`clean_variable_spelling()`，把清理字典数据框指定到`wordlists =`。`spelling_vars =`参数可用于指定字典数据框中哪列中存放拟清洗列名值（默认为第三列），或者可以设置为`NULL`使字典应用于拟清洗数据框的所有字符和因子列。请注意，此功能可能要运行很长时间。
...
linelist <- linelist %>% 
  linelist::clean_variable_spelling(
    wordlists = cleaning_dict,
    spelling_vars = "col",        # 包含拟清洗列名的字典数据框列名，默认是字典的第三列
  )
```
...
现在滚动右侧看看各列值发生了怎么样的变化 - 特别是`gender`（小写到大写），并且所有症状列都从是/否转换为 1/0。
...
请注意，清理字典中的列名称必须与清理脚本中*此时*的名称相对应。有关详细信息，请参阅 [linelist包在线参考资料](https://www.repidemicsconsortium.org/linelist/reference/clean_data.html)。
...
在这里，我们讨论一些从数值列创建分组的特殊方法。常见的例子包括年龄组别、实验室数据分组等。这里我们将讨论：

* `age_categories()`，来自**epikit**包
* `cut()`, 来自**base** R
* `case_when()`
* 分位数，可由`quantile()`和`ntile()`生成
...
本例中，将用`age_years`列创建`age_cat`列。
...
<<<<<<< HEAD
首先，检查数据的分布，以确定适当的分割点。请参阅[ggplot 基础知识]章节。
=======
首先，检查数据的分布，以确定适当的切点。请参阅[ggplot 基础知识]章节。
>>>>>>> ba0bf67d59259e5babcdc2f07ddf54400c9eed9a
...
<span style="color: orange;">**_注意_** 有时，数值变量会以“字符”类导入。这一般发生在以下情况下：如果某些值中存在非数字字符，例如年龄录入“2 个月”，或者（取决于您的 R 语言环境设置）如果在小数位使用逗号（例如“4， 5”表示四年半）。</span>
...
可使用**epikit**包的`age_categories()`函数轻松对数值列进行分类和标记（注意：此函数也可以应用于非年龄数字变量）。作为福利，输出列自动是一个有序因子。

以下是所需的输入：

* 一个数值向量（列）
* `breakers =`参数-提供新组的数值分界点
...
# 简单的例子
################
pacman::p_load(epikit)                    # 加载包

linelist <- linelist %>% 
  mutate(
    age_cat = age_categories(             # 创建新列
      age_years,                            # 创建分组的原数值列
      breakers = c(0, 5, 10, 15, 20,        # 分界点
                   30, 40, 50, 60, 70)))
...
<<<<<<< HEAD
默认情况下，您指定的分界值默认是下限 - 也就是说，这个界值包含在“较高”组中，或者说新创建组的下限/左侧是开区间。如下所示，您可以将每个分界值加 1，这样上限/右侧就会是开区间。
=======
默认情况下，您指定的分界值默认是下限 - 也就是说，这个界值包含在“较高”组中，或者说新创建组的低值/左侧是开区间。如下所示，您可以将每个分界值加 1，这样高值/右侧就会是开区间。
>>>>>>> ba0bf67d59259e5babcdc2f07ddf54400c9eed9a
...
可使用`separator = `调整标签分隔符。默认为“-”

可使用`ceiling =`参数调整处理最高数字的方式。设置上限截止参数`ceiling = TRUE`后，提供的最高分界值是“天花板”，并且不会创建类别“XX+”。任何高于最高分界值（或已经定义`upper =`值）的值都归类为`NA`。下面是一个带有`ceiling = TRUE`的示例，因此不存在XX+组别，并且将 70（最高分界值）以上的值指定为`NA`。
...
linelist <- linelist %>% 
  mutate(
    age_cat = age_categories(
      age_years, 
      breakers = c(0, 5, 10, 15, 20, 30, 40, 50, 60, 70),
      ceiling = TRUE)) # 70是天花板，所有大于70的值都变为NA
...
或者不用`breakers =`，可提供所有的`lower =`，`upper =`以及`by =`：

* `lower =` 想设置的最低数字 - 默认为 0
* `upper =` 想设置的最高数字
* `by =`    组间隔年数
...
更多信息，请参阅相关函数帮助 （在R控制台输入运行`?age_categories`即可）。 
...
`cut()`是**base** R里与`age_categories()`功能类似的函数，但`age_categories()`开发出来简化这个工作。它与`age_categories()`一些明显的不同之处在于：
...
`cut()`的基本语法是首先提供拟分组的数值列 (如 `age_years`)，然后是*breaks*参数（可为一个由分界值构成的`c()`数值向量）。`cut()`处理后，产生的列是一个有序因子变量。
...
默认情况下，`cut()`进行分组时，右/上限侧为“开放”且包含（而左/下限侧为“封闭”或不包含）。这与`age_categories()`函数相反。默认标签用符号“(A, B]”表示，这意味着  不包含A，但包含B。**通过提供`right = FALSE`参数可调整为左闭右开**。
...
因此默认情况下，“0”值被排除在最低组之外，并归类为`NA`! “0”值可能是编码为 0 岁的婴儿，所以要小心！要更改此设置，请添加参数，`include.lowest = TRUE`以便所有“0”值都包含在最低组中。最低类别的自动生成的标签将是“[A,B]”。请注意，如果您包含`include.lowest = TRUE`和 `right = TRUE`两个参数，则两端极值包含（全闭区间）将应用于最高分界点值和组别，而不是最低组别。
...
您可以使用`labels =`参数提供自定义标签的向量。由于这些是手动编写的，因此请务必小心以确保组标签准确无误！可用下面的方法列出交叉汇总表来验证。

应用`cut()`把`age_years`换算成新`age_cat`分组变量的示例如下：
...
**请检查你的工作！！！** 通过列出数值列和组别列的交叉汇总表来验证每个年龄值是否已分配给正确的组别。检查边界值的分配（例如如果相邻类别是 10-15 和 16-20，请核查一下15分在哪边）。
...
```{r}
# 数值和组别的交叉汇总表 
table("Numeric Values" = linelist$age_years,   # 指定标目便于识别
      "Categories"     = linelist$age_cat,
      useNA = "always")                        # 别忘记检查NA值
```
...
您可能想为`NA`值分配一个诸如“Missing”之类的标签。因为新列是因子类（限定值），不能简单地使用`replace_na()`来转换，此类值不被接受。取而代之，可按照[因子]章节中介绍的用**forcats**包`fct_explicit_na()`函数来处理。
...
要快速创建和标签向量，请使用如下所示的方法。有关`seq()` 和 `rep()`更多信息请参阅[R基础知识]章节。
...
在普遍的理解中，“分位数”或“百分位数”通常是指一个值及低于该值的一定比例的集合。例如，`linelist`数据框中年龄的第 95 个百分位数将是低于该年龄的 95%百分位值 的年龄集合。

然而，在比较常用的“四分位数”和“十分位数”也可以指将数据组平均分为 4 或 10 组（注意分界值数会比组数多1，四分位数默认是0，25%，50%，75%，100%五个值）。
...
要求分位数值，可使用**base** R中**stats**包的`quantile()`。提供一个数值向量（例如数据集中的一列）和范围从 0 到 1.0 的数字概率值向量作为函数参数。百分位数值以数值向量返回。可通过输入运行`?quantile`来查看该函数统计方法细节。

* 如果数向量中含有任何缺省值，最好设置 `na.rm = TRUE`参数
* 设置`names = FALSE`参数可返回一组无列名的数值向量百分位数结果
...
您可以将`quantile()`的运算结果用作`age_categories()`或`cut()` 分界值 。下面我们用`cut()`创建一个新列`deciles`,其中`cut()`分界值由`age_years`求`quantiles()`产生。下面，我们用**janitor**包的`tabyl()`显示结果，查看产生的百分位数（请参阅[描述性表格]章节）。请注意它们在每组中的比例并不完全是10%。
...
```{r}
linelist %>%                                # 以inelist数据框开始
  mutate(deciles = cut(age_years,           # 分割age_years创建deciles新列
    breaks = quantile(                      # 用quantile()定义分界值
      age_years,                               # 处理age_years变量
      probs = seq(0, 1, by = 0.1),             # 0.0到1.0，步长为0.1
      na.rm = TRUE),                           # 忽略缺省值
    include.lowest = TRUE)) %>%             # cut()函数定义包含0岁
  janitor::tabyl(deciles)                   # 传递到交叉汇总表显示出来
```
...
另一种创建数值分组的工具是`dplyr`包的`ntile()`函数，可数据分成 n个*大小均匀的组*。*但请注意：与`quantile()`不同，相同值可能分配到多个组。*需为函数提供一个数值向量和拟分成的组数。创建的新列的值只是“数字”组数（例如1到 10），而不是像`cut()`那样返回的是值的范围。
...
使用**dplyr**包的`case_when()`函数也可以从数值列创建分组变量，但使用**epikit**包的`age_categories()`或`cut()`函数可能更容易，因为它们会自动创建一个有序因子。

如果使用case_when()，请查看本页前面“重新编码值”部分中所述的正确使用方法。另请注意，所有右侧值都必须是同一类。因此，如果在右侧产生`NA`，应写成“Missing”或使用特殊`NA`值`NA_character_`。

### Add to pipe chain {.unnumbered}  
### 添加到管道链 {.unnumbered}  

Below, code to create two categorical age columns is added to the cleaning pipe chain:  
下面，将创建两个年龄分组变量的代码添加到数据清洗管道链中：
...
手动逐行添加行很乏味，但可以使用**dplyr**包的`add_row()`函数来完成。请记住，每一列必须只包含一类（字符、数值、逻辑等）的值。因此，添加一行时需要注意每列的细微差别并保证与原列类相同。
...
使用`.before`和.`after`可指定要添加的行的位置。`.before = 3`将新行放在当前第三行之前。默认是将行添加到末尾，未指定的列将设为缺省值（`NA`）。
...
新的*行号*可能看起来有点奇怪（“...23”），但预先存在的行的行号*已经*改变。因此，如果使用该命令两次，请仔细检查/测试新插入的部分。

If a class you provide is off you will see an error like this:  
如果提供的类是不一致的，会显示以下错误信息：
...
可以通过将一个数据框的行拼接到另一个数据框的尾部来将数据集组合在一起，此操作可由**dplyr**包的`bind_rows()`函数处理。请参阅[拼接数据]章节了解更多信息。
...
清洗列并重新编码值后，典型的清理步骤是使用**dplyr**动词`filter()`去*筛选*数据框的特定行。

在`filter()`中，对于数据框中要保留的那行指定的逻辑条件必须为`TRUE`。下面我们展示如何根据简单和复杂的逻辑条件来筛选行。
...
这个简单的例子将`linelist`数据框重新定义为它本身，筛选了满足逻辑条件的行。**仅保留那些圆括号中逻辑语句评估为`TRUE`的行才被保留。**

本例中，逻辑语句是`gender == "f"`，它检查`gender`列中的值是否等于“f”（区分大小写）。

筛选前，`linelist`数据集的行数是`r nrow(linelist)`。
...
要筛选掉有缺省值的行是很常见的。抵制编写`filter(!is.na(column) & !is.na(column))`此类代码的冲动，而使用为此目的定制的**tidyr**包里的`drop_na()`函数。如果使用空括号运行，将删除具有*任何*缺省值的行。或者，可以指定特定列名来评估缺省与否，或使用[上面](#clean_tidyselect)提到的“tidyselect”辅助函数。
...
在数据框中，每一行通常都有一个“行号”（在 R 查看器中查看时）出现在第一列的左侧。它本身不是数据中真正的列，但可以在`filter()`语句中引用。

要按“行号”筛选，可用**dplyr**包的`row_number()`。使用时将不带参数的`row_number()`作为逻辑筛选语句的一部分。通常，您将使用`%in%`运算符和一系列数值作为逻辑语句的一部分，如下所示。要查看*前*N行，还可以使用**dplyr**包里专用函数`head()`。
...
```{r, eval=F}
# 查看前100行数据
linelist %>% head(100)     # 用tail()查看后n行数据
...
还可将数据框通过管道传输到**tibble**包的`rownames_to_column()`函数（不要在括号中放置任何内容）将行号转换为真正的列值。
...
可以用以下操作符构建更复杂的逻辑语句：括号`( )`，逻辑或`|`，逻辑非`!`，`%in%`以及逻辑与`&`等。一个示例如下：

注意：可在逻辑条件前使用`!`运算符来否定它。例如，如果某列*无*缺省值，则`!is.na(column)`将返回“TRUE”。再如：如果某列中的值未出现在`c("a", "b", "c")`向量时，则将返回“TRUE”。
...
下面是一个用于创建发病日期的直方图的简单单行命令。请注意，此原始数据集中还包含 2012-2013 年的第二次较小爆发。**为便于分析，拟把早期暴发数据删除。**
...
可以只通过`date_onset`筛选201年6月之后的数据行吗？**注意！运行`filter(date_onset > as.Date("2013-06-01")))`代码会筛掉后续流行中的发病日期含缺省值的行。
...
<span style="color: red;">**_危险：_** 筛选大于（>）或小于（<）某日期或数值的同时会筛选掉含缺省值（`NA`）的行！这是因为`NA`被当做无限大或无限小处理。</span>

*（更多有关处理日期和**lubridate**包的信息请参阅[处理日期] 章节）*
...
```{r}
table(Hospital  = linelist$hospital,                     # 医院名称
      YearOnset = lubridate::year(linelist$date_onset),  # 发病时间年份
      useNA     = "always")                              # 显示缺省值信息
```
...
如何设置筛选条件把第一次疫情（2012 年和 2013 年）数据以从数据集中删除？我们发现：

* 2012年和2013年的第一次疫情发生在A医院、B医院，港口医院也有10例。
* A和B医院在第二次疫情中没有病例，但港口医院有。

We want to exclude:  
* ` nrow(linelist %>% filter(hospital %in% c("Hospital A", "Hospital B") | date_onset < as.Date("2013-06-01")))` rows with onset in 2012 and 2013 at either hospital A, B, or Port:  
...
需排除：  
* A、B和港口医院2012和2013年发病数据有The `r nrow(linelist %>% filter(hospital %in% c("Hospital A", "Hospital B") | date_onset < as.Date("2013-06-01")))` 行：  
  * 排除2012和2013年发病的 `r nrow(linelist %>% filter(date_onset < as.Date("2013-06-01")))` 行
  * 排除A和B医院发病日期含缺省值的` nrow(linelist %>% filter(hospital %in% c('Hospital A', 'Hospital B') & is.na(date_onset)))` 行 
  * **不要** 排除发病日期含缺省值的其他 `r nrow(linelist %>% filter(!hospital %in% c('Hospital A', 'Hospital B') & is.na(date_onset)))`行。  


We start with a linelist of ` `nrow(linelist)`. Here is our filter statement:  
从含有`r nrow(linelist)`行数据的linelist数据框开始。下面是筛选条件语句：
...
```{r}
linelist <- linelist %>% 
  # 保留2013年6月1日以后发病的观测或者非A和B医院报告且发病时间为缺省值的观测
  filter(date_onset > as.Date("2013-06-01") | (is.na(date_onset) & !hospital %in% c("Hospital A", "Hospital B")))
...
再次生成交叉汇总表时发现：我们看到A和B医院数据被完全删除，2012年和2013年的港口医院的10 个病例被删除，所有其他值都相同 - 结果正确。
```{r}
table(Hospital  = linelist$hospital,                     # hospital name
      YearOnset = lubridate::year(linelist$date_onset),  # year of date_onset
      useNA     = "always")                              # show missing values
...
多个筛选条件语句可以（用逗号分隔）置于一个筛选命令中，或者为了清楚起见，通过管道传递到单独的`filter()`命令中。

*注意：一些读者可能会注意到，仅筛选`date_hospitalisation`会更容易，因为它是 100% 完整的，没有缺省值。这是真的。但是这里用`date_onset`主要是为了演示复杂的筛选条件设置。*
...
筛选也可以作为一个独立的命令（不是管道链的一部分）来运行。与其他`dplyr`动词一样，在这种情况下，第一个参数必须是数据集。
...
经常需要快速查看几列变量的几条记录。**base** R的`View()`函数可在Rstudio里打印显示数据框。

View the linelist in RStudio:  
在Rstudio里查看`linelist`数据集
...
**用dplyr的`filter()`和`select()`函数：**
将数据集用`filter()`筛选保留特定行和用`select()`保留某此列，然后置于 `View()`函数中查看。比如，要查看3个指定病例的发病时间、入院日期等。
```{r, eval=F}
View(linelist %>%
...
要在一行内运算，可使用**dplyr**包的`rowwise()`函数，具体请参阅在线函数使用说明 [逐行计算](https://cran.r-project.org/web/packages/dplyr/vignettes/rowwise.html)。
...
例如，下面的代码应用`rowwise()`为linelist数据框中的每一行创建了一个新列，这列汇总计算了指定症状列值为“yes”的列数之和。这些指定的列名用`c()`连接为一个向量提供给`sum()`函数。`rowwise()`本质上是一种特殊的`group_by()`，因此当完成逐行操作后最好用`ungroup()`解除（[分组数据]章节）。
...
linelist %>%
  ...
  mutate(num_symptoms = sum(c(fever, chills, cough, aches, vomit) == "yes")) %>% 
  ungroup() %>% 
  select(fever, chills, cough, aches, vomit, num_symptoms) # 为了方便展示
```
...
当要指定拟计算的列时，可能需要使用本章中`select()`部分中介绍的“tidyselect”辅助函数。只需要进行一项调整（因为不是在**dplyr**包的诸如`select()`或`summarise()`之类的函数内使用）。
...
将列指定条件置于**dplyr**包的`c_across()`函数中。这是因为`c_across`（[文档](https://dplyr.tidyverse.org/reference/c_across.html)）是专门为`rowwise()`使用而设计的。例如，以下代码：
...
* 应用`rowwise()`来使接下来的操作（`sum()`）在行内进行（而不是整列求和）
* 创建新列 `num_NA_dates`，定义为每行中列名包含“date”且`is.na()`评估为TRUE（即是缺省值）的列数
* `ungroup()` 是用于消除`rowwise()`对后续步骤的影响
...
linelist %>%
  ...
  mutate(latest_date = max(c_across(contains("date")), na.rm=T)) %>% 
  ungroup() %>% 
  select(latest_date, contains("date"))  # 为了便于展示
```
...
使用**dplyr**包的`arrange()`函数可按列值进行排序。

简单地把拟排序列的列名列出来。如果要排序任何*分组*操作过的数据的话，请指定`.by_group = TRUE`参数（请参阅[分组数据]章节）。

默认情况下，列将按“升序”排序（适用于数字列和字符列）。若要“降序”排列，可通过将拟排序变量置于`desc()`中解决。

在制作[展示表格]、用`slice()`获取每组的“顶部”行或按出现顺序设置因子级别顺序时，用`arrange()`对数据进行排序特别有用。

例如，要将`linelist`数据集按`hospital`变量升序排列，然后按`date_onset`变量降序排序，会用到如下代码：

...
R项目可以将您的工作打包到一个可移植的、独立的文件夹中。在项目中，所有相关的脚本、数据文件、图形/输出和历史都存储在子文件夹中。重要的是，*工作目录*是项目的根文件夹。
...
使用R的一种常见、有效且可靠的方法是将以下三个元素组合起来： 一个独立的工作项目托管在一个R项目中。每个元素在下面的章节中进行描述。
